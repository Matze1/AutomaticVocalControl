...
The scipy.optimize package for Python deals with optimisation tasks for example the algorithmic minimisation of a problem according to the result of a self defined function.\\
...
To obtain a good initial guess the scipy.optimize.brute method was used that tries every value for each parameter in a predefined range with a predefined step size. As just an approximation to the final solution was required large step sizes suiting the different ranges were set. Therefore the scipy.optimize.brute algorithm just had to deal with three to six possibilities per parameter.\\
